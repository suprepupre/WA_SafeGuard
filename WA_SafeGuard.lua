...
----------------------------------------------------------------
-- LibCompress (ships with WeakAuras)
----------------------------------------------------------------
local LC, LCE
...
----------------------------------------------------------------
-- Compact serializer (buffer-based, no string concat spam)
----------------------------------------------------------------
local B, N
...
-- Strip stuff WeakAuras regenerates on load
local function CleanCopy()
    local copy = {}
    for k,v in pairs(WeakAurasSaved) do
        if k ~= "registered" and k ~= "tempIconCache"
           and k ~= "iconCache" then
            copy[k] = v
        end
    end
    return copy
...
local function DoBackup(silent)
    ...
    if not ok or not raw or #raw < 10 then
        if not silent then print(P.."|cffff0000Serialize failed|r") end
        return false
    end
    ...
    if not silent then
        ...
        if compressed then
            ...
            print(P..format("|cff00ff00Backup OK|r  %d auras  %dKB (no compression lib)",
                NumAuras(), rawKB))
        end
    end
    return true
...
local function DoRestore()
    ...
    if WA_SafeGuardDB.z then
        if not InitLC() then
            print(P.."|cffff0000Need LibCompress to decompress (load WeakAuras first)|r")
            return false
        end
    ...
    if not data or type(data.displays) ~= "table" then
        print(P.."|cffff0000Backup data invalid|r")
        return false
    end
...
----------------------------------------------------------------
-- Boot logic (delayed 2s after login)
----------------------------------------------------------------
...
frame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_ENTERING_WORLD" and not booted then
        booted = true
        -- delay 2s so login isn't blocked
        local wait = CreateFrame("Frame")
        local el = 0
        wait:SetScript("OnUpdate", function(self2, dt)
            el = el + dt
            if el < 2 then return end
            ...
            if IsBroken() then
                ...
                if HasBackup() then
                    ...
                    print(P.."|cffff0000No backup yet (first install?)|r")
                    print(P.."Restore your auras manually, then /reload")
                    print(P.."This addon will protect them from now on.")
                end
            else
                DoBackup()
            end
    ...
    elseif event == "PLAYER_LOGOUT" then
        -- update backup on clean logout (silent)
